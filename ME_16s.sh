...
EOF
        ...
        read -p "是否继续从3'端裁剪？[默认: n] (y/n): " trim_3p
        trim_3p=${trim_3p:-n}
...
EOF
        ...
        echo "===== 引物序列设置 [默认515F-806R(诺禾)]====="
        read -p "请输入正向引物5'端序列front-f [默认515F: GTGCCAGCMGCCGCGGTAA]: " forward_primer
        forward_primer=${forward_primer:-"GTGCCAGCMGCCGCGGTAA"}
        read -p "请输入反向引物5'端序列front-r [默认806R: GGACTACHVGGGTWTCTAAT]: " reverse_primer
        reverse_primer=${reverse_primer:-"GGACTACHVGGGTWTCTAAT"}
        trim_cmd1=$(
            cat <<EOF
qiime cutadapt trim-paired \
    --i-demultiplexed-sequences paired-raw-demux.qza \
...
EOF
        ...
        step_commands+=("$trim_cmd1")
        add_to_history "$trim_cmd1"
        read -p "是否继续从3'端修剪（adapter_f/adapter_r）？[默认: 否] (y/n): " use_adapter
